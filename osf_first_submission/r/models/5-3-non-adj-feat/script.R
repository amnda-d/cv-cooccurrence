...
v_model_rand = bf(pair_count_v | subset(isVow) ~ offset(s1_freq + s2_freq) + log_c_inv + log_v_inv + sim_new + ident_new + (ident_new + sim_new | q | language), family=negbinomial)
...
sample_rand_langs_mod <- brm(
  ...
  file='r/models/5-3-non-adj-feat/model'
)
