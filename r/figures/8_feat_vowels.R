...
p <- ggplot() +
  stat_ellipse(data=data, aes(x = v_sim, y = v_ident), level=0.95, fill='#CC79A7', color='#CC79A7', geom='polygon') +
  stat_ellipse(data=data, aes(x = v_sim, y = v_ident), level=0.75, fill='#0072B2', color='#0072B2', geom='polygon') +
  stat_ellipse(data=data, aes(x = v_sim, y = v_ident), level=0.5, fill='#009E73', color='#009E73', geom='polygon') +
  geom_point(data=lang_intervals, aes(x=sim_coef_v, y=ident_coef_v)) +
  geom_smooth(data=data %>% filter(.draw < 25), aes(x=v_sim, y=v_ident, group = .draw), method = "lm", se = F, size = 0.25, color='#3D3D3D') +
  geom_hline(aes(yintercept=0), lty=2, color='black') +
  geom_vline(aes(xintercept=0), lty=2, color='black') +
  labs(x="Vowel Similarity", y="Vowel Identity") +
  theme_light()

y_margin <- axis_canvas(p, axis='y') + geom_density(data=data, aes(y=v_ident), fill='#969696') + geom_hline(yintercept=mean(data$v_ident))
x_margin <- axis_canvas(p, axis='x') + geom_density(data=data, aes(x=v_sim), fill='#969696') + geom_vline(xintercept=mean(data$v_sim))
...
ggsave('r/figures/8_feat_vowels.png', width=5, height=5, units='in')
