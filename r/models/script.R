...
file <- args[1]
name <- unlist(strsplit(file, '.', fixed=TRUE))[1]
dir.create(file.path('r/models/', name), showWarnings = FALSE)
print(name)

library(brms)
library(bayestestR)

data <- read.csv(paste('data/r_data/', file, sep=''))
...
sample_rand_langs_mod <- brm(
  ...
  file=paste('r/models/', name, '/model', sep='')
)
