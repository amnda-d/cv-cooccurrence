...
data <- data %>% mutate(family = replace(family, family == 'indo-european', subfamily[family == 'indo-european']))
data <- data %>% mutate(family = replace(family, family == 'uralic', subfamily[family == 'uralic']))
...
sample_rand_langs_mod <- brm(
  ...
  file='r/models/feat-family-uralic/model'
)
